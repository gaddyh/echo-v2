...
3. **Start onboarding.** `start_onboarding` normalizes the phone, creates a `pending` user row, and asks for the user's name.
4. **Name response.** `handle_name_response` stores the first name (still `pending`) and spawns a background `asyncio.create_task(_provision_and_poll)`.
5. **Provision Green instance.** The background task generates a 32-byte `webhook_token`, calls `GreenProvisioner.create_connection`, and stores the returned `ConnectionRef` + `ProviderCredentials` with `webhook_token_hash = sha256(token)` and status `PROVISIONING`.
6. **Single lifecycle poll.** `_poll_until_authorized` polls `GreenClient.get_state_instance` in one loop. On `notAuthorized` it sends the OTP once; on `authorized` it completes onboarding; on timeout it distinguishes "instance never ready" (`failed`) from "OTP sent but user didn't authorize" (remains `pending`).
7. **Get OTP.** When the poll first sees `notAuthorized`, it calls `get_authorization_code(id_instance, api_token, phone_int)` and sends the 8-digit code to the user via the bot with Hebrew instructions.
...
9. **Welcome.** `handle_connection_established` sets `onboarding_status='active'` and sends the welcome message using the name already collected.
...
- **Name before provisioning.** The name is collected during onboarding (before Green provisioning) so the welcome message can use it. `update_first_name` no longer sets `onboarding_status='active'` — only authorization completion does that.
- **Onboarding states: `pending` → `active` (or `failed`).** The `connected` onboarding status is gone — connection state is owned by `WhatsAppConnection` (`PROVISIONING` → `PAIRING_REQUIRED` → `CONNECTED`).
- **Single lifecycle poll.** Replaces the previous two-phase poll (`_wait_for_instance_ready` + `_poll_for_authorization`) with one loop that tracks the instance through its lifecycle.
- **Background provisioning.** Instance creation runs in an `asyncio` background task so the webhook returns immediately.
- **OTP preferred over QR.** Green's `getAuthorizationCode` (8-digit, ~2.5 min) is used rather than QR scanning.
- **Polling fallback.** Because `stateInstanceChanged` is unreliable for OTP-based linking, the poll is the reliable completion path.
...
- Idempotent: pending users get name prompt re-sent or OTP re-sent, not a second instance; active users no-op.
- Instance creation failure → status `failed` + user-facing error message.
- OTP failure → status `failed` + user-facing error message.
- Poll timeout with instance ready (OTP sent) → user remains `pending`; `_OTP_TIMED_OUT` message; user can re-send by texting "קוד".
- Poll timeout with instance never ready → status `failed` + user-facing error message.
