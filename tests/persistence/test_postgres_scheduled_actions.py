...
# Fixed UUIDs for deterministic tests (the DB id column is UUID, not TEXT).
_ACT1 = str(uuid.uuid5(uuid.NAMESPACE_DNS, "act-1"))
_ACT2 = str(uuid.uuid5(uuid.NAMESPACE_DNS, "act-2"))
_A1 = str(uuid.uuid5(uuid.NAMESPACE_DNS, "a1"))
_A2 = str(uuid.uuid5(uuid.NAMESPACE_DNS, "a2"))
_A3 = str(uuid.uuid5(uuid.NAMESPACE_DNS, "a3"))
...
async def test_save_and_get(scheduled_actions_repo, session_factory):
    ...
    fetched = await scheduled_actions_repo.get(_ACT1)
    assert fetched is not None
    assert fetched.id == _ACT1
...
async def test_get_returns_none_for_missing(scheduled_actions_repo):
    assert await scheduled_actions_repo.get(str(uuid.uuid4())) is None


async def test_list_pending_sorted_by_execute_at(scheduled_actions_repo, session_factory):
    user_id = await insert_user(session_factory)
    await scheduled_actions_repo.save(
        _make_action(user_id, id=_A2, execute_at=datetime(2026, 9, 5, 10, tzinfo=timezone.utc))
    )
    await scheduled_actions_repo.save(
        _make_action(user_id, id=_A1, execute_at=datetime(2026, 9, 5, 8, tzinfo=timezone.utc))
    )
    await scheduled_actions_repo.save(
        _make_action(user_id, id=_A3, status=ScheduledActionStatus.SUCCEEDED)
    )
    pending = await scheduled_actions_repo.list_pending(user_id)
    assert [a.id for a in pending] == [_A1, _A2]
...
async def test_cancel_pending(scheduled_actions_repo, session_factory):
    user_id = await insert_user(session_factory)
    await scheduled_actions_repo.save(_make_action(user_id))
    assert await scheduled_actions_repo.cancel(_ACT1, user_id) is True
    action = await scheduled_actions_repo.get(_ACT1)
...
async def test_cancel_wrong_user_returns_false(scheduled_actions_repo, session_factory):
    ...
    assert await scheduled_actions_repo.cancel(_ACT1, other_user) is False
...
async def test_cancel_non_pending_returns_false(scheduled_actions_repo, session_factory):
    ...
    assert await scheduled_actions_repo.cancel(_ACT1, user_id) is False
...
async def test_recover_stale_resets_old_in_progress(scheduled_actions_repo, session_factory):
    ...
    assert recovered == 1
    assert (await scheduled_actions_repo.get(_ACT1)).status is ScheduledActionStatus.PENDING
    assert (await scheduled_actions_repo.get(_ACT1)).claimed_at is None
    assert (await scheduled_actions_repo.get(_ACT2)).status is ScheduledActionStatus.IN_PROGRESS
