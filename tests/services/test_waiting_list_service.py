...
async def test_set_label_returns_updated():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)

    result = await service.set_label(session_id, USER_ID, active_id, "red")
    assert result is not None
    assert result.outcome == "updated"
    assert result.color_label == "red"


async def test_set_label_invalid_session_returns_none():
    service, active_repo, _, _, _ = _make_service()
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)
    result = await service.set_label("invalid", USER_ID, active_id, "red")
    assert result is None


async def test_set_label_not_found_for_missing_active():
    service, _, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    result = await service.set_label(session_id, USER_ID, "missing-id", "red")
    assert result is not None
    assert result.outcome == "not_found"
    assert result.color_label is None


async def test_set_label_not_found_for_other_user():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    other_user = "user-other"
    other_chat = "972500000099@c.us"
    ...
    other_active = await active_repo.get(user_id=other_user, chat_id=other_chat)
    result = await service.set_label(session_id, USER_ID, other_active.id, "red")
    assert result is not None
    assert result.outcome == "not_found"


async def test_set_label_creates_contact_if_missing():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)

    await service.set_label(session_id, USER_ID, active_id, "blue")
    phone = CHAT_ID.split("@")[0]
    contact = await service._contact_repo.find_by_phone(USER_ID, phone)
    assert contact is not None
    assert contact.color_label == "blue"


async def test_set_label_none_clears_label():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)

    await service.set_label(session_id, USER_ID, active_id, "red")
    result = await service.set_label(session_id, USER_ID, active_id, None)
    assert result is not None
    assert result.outcome == "updated"
    assert result.color_label is None


async def test_set_label_preserves_starred():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)

    await service.set_starred(session_id, USER_ID, active_id, True)
    await service.set_label(session_id, USER_ID, active_id, "green")
    phone = CHAT_ID.split("@")[0]
    contact = await service._contact_repo.find_by_phone(USER_ID, phone)
    assert contact is not None
    assert contact.is_starred is True
    assert contact.color_label == "green"


# --- set_tags tests ---------------------------------------------------------


async def test_set_tags_returns_updated():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)

    result = await service.set_tags(session_id, USER_ID, active_id, ["work", "urgent"])
    assert result is not None
    assert result.outcome == "updated"
    assert result.tags == ["work", "urgent"]


async def test_set_tags_invalid_session_returns_none():
    service, active_repo, _, _, _ = _make_service()
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)
    result = await service.set_tags("invalid", USER_ID, active_id, ["work"])
    assert result is None


async def test_set_tags_not_found_for_missing_active():
    service, _, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    result = await service.set_tags(session_id, USER_ID, "missing-id", ["work"])
    assert result is not None
    assert result.outcome == "not_found"
    assert result.tags == []


async def test_set_tags_creates_contact_if_missing():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)

    await service.set_tags(session_id, USER_ID, active_id, ["work"])
    phone = CHAT_ID.split("@")[0]
    contact = await service._contact_repo.find_by_phone(USER_ID, phone)
    assert contact is not None
    assert contact.tags == ["work"]


async def test_set_tags_normalizes():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)

    result = await service.set_tags(
        session_id, USER_ID, active_id, ["  Work  ", "", "work", "URGENT"]
    )
    assert result is not None
    assert result.tags == ["Work", "URGENT"]


async def test_set_tags_empty_clears_tags():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)

    await service.set_tags(session_id, USER_ID, active_id, ["work"])
    result = await service.set_tags(session_id, USER_ID, active_id, [])
    assert result is not None
    assert result.outcome == "updated"
    assert result.tags == []


async def test_set_tags_preserves_label():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)
    ...
    contact = await service._contact_repo.find_by_phone(USER_ID, phone)
    assert contact is not None
    assert contact.color_label == "red"
    assert contact.tags == ["work"]
...
async def test_list_tags_returns_distinct_sorted():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)

    await service.set_tags(session_id, USER_ID, active_id, ["work", "urgent"])
    # Add a second active with different chat to set more tags.
    other_chat = "972500000099@c.us"
    from echo_v2.ports.whatsapp import MessageDirection

    await service._chat_state_repo.upsert_on_message(
        user_id=USER_ID, chat_id=other_chat,
        direction=MessageDirection.INBOUND, observed_at=NOW, next_analysis_at=None,
    )
    await active_repo.upsert(
        user_id=USER_ID, chat_id=other_chat, target_version=1,
        result_id=RESULT_ID, waiting_since=NOW,
    )
    other_active = await active_repo.get(user_id=USER_ID, chat_id=other_chat)
    await service.set_tags(session_id, USER_ID, other_active.id, ["family", "work"])

    tags = await service.list_tags(session_id, USER_ID)
    assert tags == ["family", "urgent", "work"]


async def test_list_tags_invalid_session_returns_none():
    service, _, _, _, _ = _make_service()
    result = await service.list_tags("invalid", USER_ID)
    assert result is None
...
# --- list_items includes color_label and tags -------------------------------


async def test_list_items_includes_color_label_and_tags():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    active_id = await _setup_chat_and_active(active_repo, service._chat_state_repo)

    await service.set_label(session_id, USER_ID, active_id, "blue")
    await service.set_tags(session_id, USER_ID, active_id, ["work", "family"])

    result = await service.list_items(session_id, USER_ID)
    assert result is not None
    assert len(result.items) == 1
    item = result.items[0]
    assert item.color_label == "blue"
    assert item.tags == ["work", "family"]


async def test_list_items_defaults_no_label_no_tags():
    service, active_repo, _, _, token_service = _make_service()
    session_id, _ = await token_service.issue(USER_ID)
    await _setup_chat_and_active(active_repo, service._chat_state_repo)
